...
async function finalizeContest(contest) {
  ...
  const results = standings.map((row) => ({
    ...row,
    prize_awarded: prizesByRank[row.rank] || 0,
  }));
  ...
  for (const winner of results.filter((r) => r.rank <= 3 && r.prize_awarded > 0)) {
    try {
      ...
      await sendTelegramMessage(
        winner.telegram_id,
        `${medal} <b>Congratulations!</b> You placed <b>${place}</b> in the Mining Contest with ${winner.active_referrals} active referrals — <b>+${winner.prize_awarded} ADLX</b> has been added to your balance!`,
        { parseMode: 'HTML' }
      );
    } catch (err) {
...
async function startNewContest(settings) {
  ...
  await client.execute({
    sql: `
      INSERT INTO mining_contests (id, starts_at, ends_at, duration_days, active_referral_cycles, prize_1st, prize_2nd, prize_3rd, status)
...
function toCsv(contest) {
  const header = 'Rank,User,Telegram ID,Active Referrals,Prize Awarded (ADLX)\n';
  const rows = (contest.results || [])
    .map((r) => [r.rank, r.display_name, r.telegram_id, r.active_referrals, r.prize_awarded || 0].join(','))
    .join('\n');
  return header + rows;
}
