...
CREATE TABLE IF NOT EXISTS mining_contests (
    ...
    -- Eligibility floor per position — even the #1 spot gets NO prize
    -- if they haven't reached this many active referrals (keeps a
    -- near-empty round from paying out a prize nobody really earned).
    -- Snapshotted at creation, same reasoning as everything else here.
    min_active_referrals_1st INTEGER NOT NULL DEFAULT 10,
    min_active_referrals_2nd INTEGER NOT NULL DEFAULT 8,
    min_active_referrals_3rd INTEGER NOT NULL DEFAULT 6,
